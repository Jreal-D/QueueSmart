...
class CustomerApp {
  constructor() {
    this.apiBaseUrl = "https://queuesmart.onrender.com"
  ...
  async checkWaitTime() {
    ...
    try {
      ...
      if (response.ok) {
        this.displayWaitTimeResult(
          data,
          simulatedQueueLength,
          isCurrentTime,
          predictionHour
        )
      } else {
        throw new Error(
          `API Error: ${data.message || data.details || "Unknown error"}`
        )
      }
    } catch (error) {
  ...
  displayWaitTimeResult(data, queueLength, isCurrentTime, predictionHour) {
    ...
    const quickCheckCard = document.querySelector(".quick-check .card")
    quickCheckCard.setAttribute("data-status", "result-visible")
    ...
    if (!isCurrentTime) {
      setTimeout(() => {
        ...
        if (currentHour < 8) {
          message =
            "Bank opens at 8:00 AM. This prediction is for opening time."
        } else {
          message =
            "Banking hours are 8:00 AM - 4:00 PM. This prediction is for tomorrow morning."
        }
        Toastify({
          text: message,
          duration: 5000,
  ...
  showTravelTimeModal() {
    ...
    document.querySelectorAll(".travel-option").forEach((option) => {
      option.addEventListener("click", (e) => {
        ...
        document
          .querySelectorAll(".travel-option")
  ...
  showQueueStatus(queueData) {
    ...
    const remainingWait = Math.max(
      0,
      queueData.originalWaitTime - elapsedMinutes
    )
    const timeToNotify = Math.max(
      0,
      queueData.notifyTime - elapsedMinutes
    ).toPrecision(2)
  ...
  rejoinQueue() {
    if (
      confirm(
        "Would you like to rejoin the queue? You'll get a new estimated wait time."
      )
    ) {
      // Simulate new wait time (in real app, this would call the API)
      const newWaitTime = Math.floor(Math.random() * 20) + 15

      const travelTime = JSON.parse(
        localStorage.getItem("queueData")
      ).travelTime
      this.lastWaitTime = newWaitTime
      this.confirmQueueJoin(travelTime)

      this.showSuccess(
        `You've rejoined the queue. New wait time: ${newWaitTime} minutes.`
      )
    }
  }
  ...
  completeService() {
    ...
    this.showSuccess(
      "Thank you for using QueueSmart! We hope we saved you time today."
    )
  }
