...
class QueueSmartDashboard {
  ...
  async checkSystemStatus() {
    try {
      ...
      this.updateSystemStatus(
        "online",
        `Live - ${response.service} v${response.version}`
      )
    } catch (error) {
      this.isConnected = false
  ...
  async loadBranches() {
    try {
      const response = await API_UTILS.makeRequest(
  ...
  async loadModelInfo() {
    try {
      const response = await API_UTILS.makeRequest(
        API_CONFIG.ENDPOINTS.MODEL_STATUS
      )
  ...
  updateModelDetails() {
    ...
    featureList.innerHTML = this.modelInfo.features
      .map(
        (feature) =>
  ...
  updateOverviewCards() {
    ...
    document.getElementById("predictionsToday").textContent =
      simulatedData.predictionsToday
    document.getElementById(
      "avgWaitTime"
    ).textContent = `${simulatedData.avgWaitTime} min`
    document.getElementById("modelAccuracy").textContent =
      simulatedData.modelAccuracy
  }
  ...
  async runTestPrediction() {
    ...
    const queueLength = parseInt(
      document.getElementById("testQueueLength").value
    )
    ...
    try {
      ...
      // Show result
      outputP.innerHTML = `
                <strong>Wait Time:</strong> ${API_UTILS.formatWaitTime(
                  response.wait_time_minutes
                )}<br>
                ...
                <strong>Estimated Service Time:</strong> ${
                  response.estimated_service_time
                }
            `
  ...
  createBranchChart() {
    ...
    const branchData = this.branches.map(() =>
  ...
  createServiceChart() {
    ...
    const serviceData = this.services.map(() =>
